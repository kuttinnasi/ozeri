<!DOCTYPE html>
<html class="no-js" lang="en">

<head>
    <meta charset="utf-8" />

    <!--====== Title ======-->
    <title>Photo Album &#8211; Fr Peter Mary Rookey &#8211; Servite Friar</title>

    <meta name="description" content="" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />

    <!--====== Favicon Icon ======-->

    <!-- CSS
    ============================================ -->
    <?php 
    include '../config.php';
    include '../includes/head-links.php';
?>
</head>

<body>
    <!--====== Header Start ======-->
    <?php include '../includes/header.php'; ?>
    <!--====== Header Ends ======-->

    <section class="about-area">
        <div class="container">
            <div class="section-content">
                <h2 class="blog-title">Photo Album</h2>
                
                <div class="album-grid">
                    <!-- Item 1 -->
                    <div class="album-item">
                        <a href="fr-rookey-and-family.php">
                            <img src="../assets/img/Fr.Rookey-family-cover.jpg" alt="Fr. Rookey & Family">
                        </a>
                        <div class="cover-photo-text">
                            <h5><a href="fr-rookey-and-family.php" style="color: inherit; text-decoration: none;">Fr. Rookey &amp; Family</a></h5>
                        </div>
                    </div>

                    <!-- Item 2 -->
                    <div class="album-item">
                        <a href="#">
                            <img src="../assets/img/Fr.Rookey-Priesthood-1941-1995-cover.jpg" alt="Fr. Rookey Priesthood">
                        </a>
                        <div class="cover-photo-text">
                            <h5>Fr. Rookey Priesthood<br>
                            (1941-1995)</h5>
                        </div>
                    </div>

                    <!-- Item 3 -->
                    <div class="album-item">
                        <a href="#">
                            <img src="../assets/img/Fr.Rookey-Priesthood-1995-2014-cover.jpg" alt="Fr. Rookey Priesthood">
                        </a>
                        <div class="cover-photo-text">
                            <h5>Fr. Rookey Priesthood<br>
                            (1995-2014)</h5>
                        </div>
                    </div>

                    <!-- Item 4 -->
                    <div class="album-item">
                        <a href="#">
                            <img src="../assets/img/Fr.-Peter-Friends.jpg" alt="Fr. Rookey & Friends">
                        </a>
                        <div class="cover-photo-text">
                            <h5>Fr. Rookey & Friends</h5>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </section>

    <!--====== Footer Start ======-->
    <?php include '../includes/footer.php'; ?>
    <!--====== Footer Ends ======-->

    <!--====== BACK TOP TOP PART START ======-->
    <!-- <a href="#" class="back-to-top"><i class="fa fa-chevron-up"></i></a> -->
    <!--====== BACK TOP TOP PART ENDS ======-->

    <!--====== Jquery js ======-->
    <?php include '../includes/script-links.php'; ?>
</body>

</html>
